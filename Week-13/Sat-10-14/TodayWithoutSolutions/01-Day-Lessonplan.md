## 13.1 Lesson Plan - Intro to Node Servers
...
* Open up the `server.js` file found in `05-Serve-Favorites/Solved` and review the code in this example. During your discussion, be sure to point out:
  ...
  * How we created the basic skeleton of a Node server (requiring: `url`, `http`, and the port number)
  ...
  * Lastly, created JavaScript functions (e.g. `displayRoot`) which dynamically creates HTML based on the page selected.
...
* Open `06-Request-Methods/server.js` in sublime.
